...
int main (){
	...
	int centro[2] = {width/2, height/2};
	int menorRes;
	if (width<height) menorRes = width; else menorRes = height;
	...
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++){
			float distCentro = sqrt(pow(centro[0] - j, 2) + pow(centro[1] - i, 2));
			distCentro = distCentro/menorRes*2*255;
			int verde = 255 - distCentro;
			if (verde < 0) verde = 0;
			int azul = 255 - verde;

			createColor(0, verde, azul, tempColor);
